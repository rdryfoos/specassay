...
- [x] T922 Write FR-DOCS-60: onboarding ladder rung one, a blank session to a first Thread Report in one sitting — **Carries**: FR-DOCS-60
- [x] T923 Consume the configured grammar as configured, everywhere a grammar is read — **Carries**: FR-GATE-110, AC-GATE-110, FR-GATE-120, AC-GATE-120, FR-GATE-130, AC-GATE-130. Commissioned 2026-09-16 from the Chalkup trial, an estate whose IDs carry a dotted grammar (`AC-5.6.1a`). Three reported defects, one class: the proof scan rewrote a test token with a hardcoded `tr '_' '-'` instead of resolving it against the registry the config produced, making every non-stock ID unreachable by any test and silently green; the definition-line pattern interpolated `id_regex` into an anchored regex without grouping it, so an alternation escaped the anchor; and the audit Rik ruled in scope found four more of the same shape (a JUnit matcher building ID forms by hardcoded rule, a configured retirement mark interpolated into `sed`, a configured mark reaching `awk` through an escape-processed `-v` assignment, and orphan scoping assuming the stock three-segment ID shape). Per the 2026-09-16 split ruling, only the three genuinely new promises mint here; the seven repairs cite the rows they restore in the 0.5.0 release notes, which is where a repair belongs, since nothing new was promised. Eleven regression tests in `test_gate_110_nonstock_grammar.py`, ten of them failing against the v0.4.13 tag and passing after, on fixtures carrying dotted IDs, an underscore-bearing ID, an alternation in `id_regex`, and a retirement mark containing a slash. The eleventh is a static guard: no configured pattern may travel through `awk -v`, which is unobservable behaviourally on a machine with only one awk (the container runs mawk, CI runs gawk, and CI is what caught it). Shipped in 0.5.0.
- [x] T924 Trim the Thread Report to a five-second read, and put the display under this registry's own governance — **Carries**: FR-THREAD-10, AC-THREAD-10. Commissioned 2026-09-17 after the first Thread Report rendered on a real estate (Chalkup `CHA-23`): every moved row appeared twice, once as a bullet and again as a table row marked changed; the family tables listed rows that had not moved beside the ones that had; and the gate log that produced the report sat open at the bottom. Three trims, all display, none touching what is true. The bullets went and the tables absorbed them; a family now lists only what moved and footnotes the rest by status; the moved rows, the off-thread list and a new caller-supplied `--receipts` appendix each render folded. *Intent Changed* and a required human tick deliberately do not fold, and that exception is in the criterion by name rather than left to a reader of the code. Eighteen tests, six of them named for the criterion, one per clause; fifteen fail against the previous script and the three that pass there pass vacuously, asserting the absence of something that version never had. `ONBOARD.md` block 11 quotes a real report, so its receipt was re-captured by replaying blocks 5 through 11 rather than edited. **Ruling, 2026-09-17 (Rik), on what this mints:** the display contract only. A shipped tool this repo's own registry did not govern was the assay office running an ungoverned instrument, and the display is the promise a reader can actually hold us to. Governing the tool's behaviour more broadly is undecided and waits until there is behaviour worth naming; minting it now would be the umbrella row declined at the 0.5.0 mint. Item three of the commission, folding the estate's own gate log, is not this repo's to make: that block is written by Chalkup's gate wrapper, and `--receipts` is the place this repo gave it to land.
