...
- FR-DIG-70 — `rowId` (stable, ordering-derived) and `candidateParent` (a LIST; inferred structure isn't bound by the registry's single-parent law) first-class on every row. Shipped 2026-08-22 evening (`docs/dig-structure-emission-handoff-v3-1-2026-08-22.md`), proven by `@covers` in `extensions/specassay-check/scripts/dig.py` (`assign_row_ids`, `attach_candidate_parent`, `dig_readme_tables`'s `_parentCandidates` marking).
  - AC-DIG-80 — proven by `test_AC_DIG_80_row_ids_stable_and_ordering_derived` and `test_AC_DIG_80_table_adjacency_candidate_parent`. Tonight emits exactly one basis, `"table-adjacency"`: the same physical README table row's own spec cell pairs with its scenario cell (data table mining already recovered at level two). Real-target confirmation: re-run against `github.com/SpecDriven/insurance-java` at 0.4.0 held the row count at 56 (purely additive: every prior field on every row byte-for-byte unchanged, only `rowId`/`candidateParent` added), emitted 17 edges into 12 distinct parents (every one of the 17 table-mined scenario rows, matching the published table exactly), and left 39 honest orphans (25 test-derived ACs, 2 readme-heading USs, 12 table spec/US rows themselves — table-adjacency correctly proposes no parent for a row that IS a spec) — see `samples/insurance-java.dig-report.json`. Test-class containment (the orphan-adopter) is explicitly deferred, named as the next rung pending a Rik ruling on its dedup-against-table-parents question.
