...
**Why the Gate is deterministic, with a receipt.** Row states move on parsed marks, named proofs, and exact-set registry checks, never on a model's opinion of a diff. That was a design assertion until it was measured. In a 163-run controlled trial, a blind LLM reviewer scoring the same work as hidden executable acceptance tests came out **uncorrelated with them (r = −0.031)**, and **of the 40 runs the tests called broken it called 29 excellent (72.5%)**. Its misses clustered: it reliably caught wrong wording and wrong branches, and it missed a notice that never fired 17 times out of 17. A Gate that asked a model whether the thread held would have passed most of the broken work. See [`docs/testing/completed/evidence-agent-thread-effect.md`](./docs/testing/completed/evidence-agent-thread-effect.md).
...
**Reading a trace-manifest in SDLC terms** (intent → build → proof → Gate → Loupe): [`docs/reading-a-manifest.md`](./docs/reading-a-manifest.md). Visual tour with screenshots: [**specassay.com/field-guide**](https://specassay.com/field-guide). **Does it work cold?** A zero-context agent on stock Spec Kit + this bundle delivered a PRD item end to end, Gate-clean: [`docs/testing/completed/evidence-cold-agent-trial.md`](./docs/testing/completed/evidence-cold-agent-trial.md). **Does it actually help an agent?** A pre-registered three-arm trial, 163 runs graded by hidden executable tests: on a task turning on stated intent the threaded arm scored 1.000 against 0.396 with no thread, and agents opened the spec exactly when the ticket needed it and never otherwise: [`docs/testing/completed/evidence-agent-thread-effect.md`](./docs/testing/completed/evidence-agent-thread-effect.md). **Want to test it on real work?** The runbook, setup through tear-out: [`docs/testing/4-real-work-test.md`](./docs/testing/4-real-work-test.md). **Something looks wrong?** [`docs/troubleshooting.md`](./docs/troubleshooting.md) — every entry taught by a real incident, not a guess.
